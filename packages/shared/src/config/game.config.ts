...
export interface GameConfig {
  ...
  readonly defaultMaxPlayersPerRoom: number
...
export const GameConfig = Config.nested(
  Config.all({
    ...
    defaultMaxPlayersPerRoom: Config.number('DEFAULT_MAX_PLAYERS_PER_ROOM').pipe(
      Config.withDefault(10)
    ),
    matchmakingMaxConcurrency: Config.number('MATCHMAKING_MAX_CONCURRENCY').pipe(
      Config.withDefault(5)
    ),
    leaderboardMaxDefinitions: Config.number('LEADERBOARD_MAX_DEFINITIONS').pipe(
      Config.withDefault(5)
    ),
    staticDataCacheTtlSeconds: Config.number('STATIC_DATA_CACHE_TTL_SECONDS').pipe(
      Config.withDefault(3600)
    ),
    leaderboardCacheTtlSeconds: Config.number('LEADERBOARD_CACHE_TTL_SECONDS').pipe(
